...
// AI / heavy-compute endpoints beyond chat: Groq vision (documents), embeddings +
// generation (rag), and ingestion. Share the stricter LLM bucket.
...
export function proxy(req: NextRequest) {
  ...
  // 2. Rate limiting (stricter for the LLM / heavy-compute endpoints)
  const isAi = AI_PREFIXES.some((p) => pathname.startsWith(p));
  const limit = isAi ? CHAT_LIMIT : GENERAL_LIMIT;
  const windowMs = isAi ? CHAT_WINDOW_MS : GENERAL_WINDOW_MS;
  const scope = isAi ? "ai" : "general";
